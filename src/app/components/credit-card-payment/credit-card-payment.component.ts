...
export class CreditCardPaymentComponent implements OnInit {
  ...
  async rentACar(){
    let payment:Payment = {nameOnTheCard:this.nameOnTheCard,cardNumber:this.cardNumber,cardCvv:this.cardCvv}
    this.cardExist = await this.isCardExist(payment)
    if(this.cardExist){
      this.payment = await((this.getFakeCardByCardNumber(this.cardNumber))) 
      this.payment.moneyInTheCard = this.payment.moneyInTheCard - this.rental.totalRentPrice
